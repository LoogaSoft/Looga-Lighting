    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(VaryingsCrystal input) : SV_Target
            {
                ...
                // Using subtraction to push the parallax into the mesh
                ...
                // 3. Core Edge Masking (Real-Time vs Fresnel)
                float NoV = saturate(dot(normalWS, input.viewDirWS));
                float fresnelMask = pow(NoV, _CoreDensity);
                
                // Divide the normal change by the world-space pixel size to make the edges distance-independent
                ...
                // We scale it down slightly (e.g., * 0.1) so your _EdgeSharpness slider still feels good
                ...
                // 5. Surface Specular & Reflections
                ...
                half3 environmentReflection = 0.0;
                    ...
                    environmentReflection = GlossyEnvironmentReflection(reflectionDir, input.positionWS, perceptualRoughness, 1.0);
                #endif
                
                // 6. Final Composite
                half3 finalColor = transmission + coreGlow + environmentReflection + specularAccumulation;
