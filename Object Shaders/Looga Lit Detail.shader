Shader "LoogaSoft/Lit Detail"
{
    Properties
    {
        ...
        [Enum(Metallic Alpha, 0, Albedo Alpha, 1)] _DetailSmoothnessTextureChannel ("Detail Smoothness Source", Float) = 0.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Keeps the Stencil tag intact for your loot/extraction outlining later
            Stencil
            {
                Ref 128
                Comp Always
            ...
            // Detail Maps (Reusing samplers to stay under the 16 sampler hardware limit)
            TEXTURE2D(_DetailBlendMap);
            TEXTURE2D(_DetailBaseMap);
            TEXTURE2D(_DetailNormalMap);
            TEXTURE2D(_DetailMetallicMap);
            TEXTURE2D(_DetailOcclusionMap);
            TEXTURE2D(_DetailMaskMap);
            TEXTURE2D(_DetailEmissionMap);
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                float _DetailBlendStrength;
                float4 _DetailBaseColor;
                float _DetailNormalScale;
                float _DetailMetallic;
                float _DetailOcclusionStrength;
                float _DetailSmoothnessTextureChannel;
                float _DetailBaseSmoothnessScale;
                float4 _DetailEmissionColor;
