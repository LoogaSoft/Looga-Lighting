...
half4 LoogaDeferredLightingFrag(Varyings input) : SV_Target
{
    ...
    finalColor += EvaluateGlobalLoogaLighting(diffuseColor, f0, perceptualRoughness, normalWS, occlusion, viewDirectionWS, NoV, mainLight.direction, mainRadiance);
    ...
    // Evaluate Additional Lights
        ...
        while (ClusterNext(clusterIterator, lightIndex))
        {
            ...
            // NEW: Call the master global switch function
            finalColor += EvaluateGlobalLoogaLighting(diffuseColor, f0, perceptualRoughness, normalWS, occlusion, viewDirectionWS, NoV, light.direction, dynRadiance);
        }
    #else
    ...
    finalColor += EvaluateLoogaDeferredMetalEnvironmentReflection(f0, metallic, perceptualRoughness, normalWS, bentNormalWS, viewDirectionWS, NoV, occlusion, positionWS, uv);
    finalColor += EvaluateLoogaMetalAmbientReflection(f0, metallic, perceptualRoughness, normalWS, bentNormalWS, viewDirectionWS, NoV, occlusion);
