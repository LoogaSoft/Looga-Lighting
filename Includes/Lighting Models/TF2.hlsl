#ifndef LOOGA_MODEL_TF2_INCLUDED
#define LOOGA_MODEL_TF2_INCLUDED

#include "Packages/com.loogasoft.loogalighting/Includes/LoogaLightingHelpers.hlsl"

float3 EvaluateLighting_TF2(float3 diffuseColor, float3 f0, float perceptualRoughness, float3 normalWS, float occlusion, float3 viewDirectionWS, float NoV, float3 lightDir, float3 lightColor)
{
    ...
    // CORRECTED: Divide by PI for energy conservation
    float3 diffuse = (diffuseColor / PI) * warpedDiffuse;
    ...
    // CORRECTED: Divide by PI here as well
    float3 rimLight = (diffuseColor / PI) * rimTerm;
...
float3 EvaluateIndirect_TF2(float3 f0, float perceptualRoughness, float occlusion, float3 viewDirWS, float3 normalWS, float3 bentNormalWS, float NoV, float3 posWS, float2 uv)
{
